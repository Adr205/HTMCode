...
class NeuralPoints(Visitor):
    ...
    def asignacionsimple(self, tree):
        ...
        # print(varName, varValue, pFor)
        
        #Añadir opcion fo
        if pFor == True:
            tablaDeVariables.addVariable(Variable(varName, 'int', varValue))
        ...
        if pFor == True:
            VC = var.value
            cuadruplos.addCuadruplo('=', var.value,None,varName)
    ...
    def ciclofor(self, tree):
        asignacion = tree.children[3].scan_values(
            lambda v: isinstance(v, Token))
        # for value in asignacion:
        #     print(value, end='\n')
        expresion = tree.children[4].scan_values(
            lambda v: isinstance(v, Token))
        # for value in expresion:
        #     print(value, end='\n')
        contador = tree.children[7].scan_values(lambda v: isinstance(v, Token))
    ...
    def np_for_2(self, tree):
        ...
        cuadruplos.addCuadruplo(contador[1][0], 'VC', contador[2], len(contVariablesTemporales))
        contVariablesTemporales.append(0)
        cuadruplos.addCuadruplo('=', 't' + str(len(contVariablesTemporales) - 1), None, 'VC')
        cuadruplos.addCuadruplo('=', 't' + str(len(contVariablesTemporales) - 1), None, contador[0])
        cuadruplos.fillCuadruplo(pSaltos.pop(), cuadruplos.contador)

    def contador(self,tree):
        varName = tree.children[0].children[0].value
        expresion = tree.children[0].children[1].children[0].value

        if expresion == '+=' or expresion == '-=' or expresion == '*=' or expresion == '/=':
            valueExpresion = tree.children[0].children[2].value
            contadores.append([varName, expresion, int(valueExpresion)])
        else:
            contadores.append([varName, expresion, 1])
    ...
    def np_logico_2(self, tree):
        ...
        if len(pOper) > 0:
            ...
            if pFor == True:
                ...
                cuadruplos.addCuadruplo('<', 'VC', 'VF', 't' + str(len(contVariablesTemporales)))
                contVariablesTemporales.append(0)
                cuadruplos.addCuadruplo('GOTOF', len(contVariablesTemporales) - 1, None, "N/A")
                pSaltos.append(cuadruplos.contador - 1)
            else:   
                # print(left_operand, operator, right_operand)
                resultType = semantic_Cube.semantic_cube[operator][left_type][right_type]
                cuadruplos.addCuadruplo(operator, left_operand, right_operand, 't' + str(len(contVariablesTemporales)))
                contVariablesTemporales.append(0)
